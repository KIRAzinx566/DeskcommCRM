...
export interface PontoDeIa {
   ...
   *
   * `usa`, quando presente, é o provider+modelo que o CÓDIGO realmente chama —
   * hardcoded, sem passar por `organizations.settings.llm` nem por binding
   * nenhum. Sem isto, a tela roda estes pontos pelo MESMO resolvedor dos
   * pontos configuráveis, e o card mostra "usando o padrão da organização"
   * com o modelo de CONVERSA da org (ex.: Claude) — quando o
...
export const PONTOS_DE_IA: readonly PontoDeIa[] = [
  ...
  },
  {
    ...
    fixo: {
      ...
      usa: { provider: "openai", modelId: "text-embedding-3-small" },
    },
    sintomaDeFalha:
