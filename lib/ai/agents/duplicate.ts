...
function versionPayloadFrom(src: Record<string, unknown>) {
  return {
    ...
    // `provisioning_origin` FICA DE FORA de propósito — junta-se a id/version_number/
    // status/published_at/superseded_at como campo que não pode vazar da origem. Ele
    // marca COMO aquela versão específica nasceu (onboarding/legacy_reconciliation), e
    // a própria migration 0227 zera a coluna via trigger sempre que o conteúdo muda —
    // uma cópia é sempre um evento de criação novo, nunca uma reconciliação.
  };
}
