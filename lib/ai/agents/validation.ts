...
export const versionPatchSchema = versionShapeSchema
  .extend({
    // Sem isto, um PATCH que só toca `followup` mandava `base_url: null` junto
    // — `.partial()` não remove `.default(null)` de um campo AUSENTE do body,
    // e um PATCH concorrente que já tinha posto o endpoint próprio da versão
    // seria apagado por um PATCH que nunca mencionou `base_url`.
    base_url: versionShapeSchema.shape.base_url.removeDefault(),
