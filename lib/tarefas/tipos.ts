 ...
 * `const X = [...] as const` e compara com o CHECK da migration 0236. Trocar
...
export interface Tarefa {
  ...
  /** ISO-8601 com offset. Nula = sem prazo — ver o cabeçalho da migration 0236. */
...
/**
 * A tarefa que o card do Kanban/dossiê mostra: a ABERTA de prazo mais
 * próximo, entre as que TÊM prazo. Pode haver várias abertas por negócio — a
 * UI nunca soma nem empilha, sempre mostra uma só, a mais urgente.
 *
 * Sem prazo fica de fora de propósito: uma tarefa "algum dia eu preciso" não
 * compete por urgência com uma que vence amanhã, e misturar as duas faria a
 * mais distante (ou a sem-prazo, se entrasse com `Infinity`) roubar o slot de
 * quem realmente vence primeiro.
 */
export function tarefaMaisProxima<T extends Pick<Tarefa, "due_date" | "status">>(
  tarefas: readonly T[],
): T | null {
  let escolhida: T | null = null;
  for (const t of tarefas) {
    if (estaEncerrada(t)) continue;
    if (t.due_date === null) continue;
    if (!escolhida || new Date(t.due_date).getTime() < new Date(escolhida.due_date!).getTime()) {
      escolhida = t;
    }
  }
  return escolhida;
