 ...
 * (`meta_webhook_override_uri|erro|em`, migration 0381, renumerada de 0311): a tela mostra "conectado,
 ...
 *
 * ─── Banco sem a migration 0381 (renumerada de 0311) não mente nem quebra ────
...
/** As três colunas do desfecho, na ordem em que a migration 0381 (renumerada de 0311) as cria. */
...
/** O erro é "a migration 0381 (renumerada de 0311) não rodou neste banco" — e não um erro de verdade. */
...
export async function registrarWebhookDaSessao(input: {
  ...
  const gravar = async (desfecho: DesfechoDoWebhookDaSessao): Promise<void> => {
    ...
    if (ehColunaDoDesfechoAusente(error.message)) {
      logger.warn(
        "migration 0381 (renumerada de 0311) não aplicada: o desfecho do webhook não foi gravado (a rota devolve o estado na resposta)",
