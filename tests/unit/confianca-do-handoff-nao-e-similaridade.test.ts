...
describe("a classe inteira: ausência de medição não vira zero nos caminhos de IA", () => {
  ...
  function arquivosDe(dir: string): string[] {
    ...
    for (const entrada of readdirSync(raiz, { withFileTypes: true, recursive: true })) {
      ...
      // `path.join` usa separador do SO — no Windows isso quebra o `.endsWith`
      // de barra que o controle positivo abaixo assume.
      achados.push(path.join(entrada.parentPath ?? raiz, entrada.name).split(path.sep).join("/"));
    }
    return achados;
  }
