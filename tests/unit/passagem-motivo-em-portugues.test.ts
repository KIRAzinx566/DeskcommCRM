  ...
  it("nenhum código cru do vocabulário aparece em `app/`, `components/` ou `hooks/`", () => {
    ...
    for (const arquivo of arquivos) {
      ...
      for (const codigo of codigos) {
        ...
        // `relative()` usa separador do SO — no Windows isso quebra a busca em
        // `liberados`, cujas chaves em USO_COMO_CHAVE usam `/` literal.
        const par = `${relative(RAIZ, arquivo).split(sep).join("/")} → ${codigo}`;
