...
const MIGRATION = "supabase/migrations/20260915213849_0271_vocabulario_de_tags.sql";
/**
 * As assinaturas como a 0271 as publicou (renumerada de 0264 — colidia com
 * migration própria do fork) — é o TEXTO daquele arquivo que os casos abaixo
 * leem, e o arquivo não muda (migration aplicada não se edita: forward-fix
 * numa nova, que é a 0336).
 */
const FUNCOES_DA_0271 = [
...
describe("fatia S4 — vocabulário de tags (contrato do que foi escrito)", () => {
  ...
  it("as DUAS origens de EXECUTE são revogadas para cada função nova", () => {
    const sql = ler(MIGRATION);
    for (const fn of FUNCOES_DA_0271) {
  ...
  it("o MANIFEST aponta a migration, como as irmãs", () => {
    ...
    expect(manifest).toContain("0271_vocabulario_de_tags");
  });
