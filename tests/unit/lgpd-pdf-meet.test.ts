...
async function rendered(data: ExportPayload) {
  ...
  // `standardFontDataUrl` é tratado como URL pelo pdfjs, não caminho de SO: num
  // checkout Windows, `join(...) + sep` termina em "\" e pdfjs recusa dizendo
  // que falta a barra final — ele só reconhece "/". Normalizar aqui, não trocar
  // `path.join` por concatenação manual, que quebraria o separador certo no Linux.
  const fonts =
    (
      join(
