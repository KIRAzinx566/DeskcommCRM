...
describe("o motivo da transferência — as duas metades do mesmo nome", () => {
  ...
  it("o trigger aceita o motivo — senão o banco recusa o encerramento com 22023", () => {
    // A ÚLTIMA definição é a que vale no banco (apêndice da 0311 (renumerada de 0266), depois do dump).
  ...
  it("a migration 0311 (renumerada de 0266) existe, está no baseline e no MANIFEST", () => {
    const arquivos = readFileSync(path.join(raiz, "supabase/migrations/MANIFEST.md"), "utf8");
    expect(arquivos).toContain("0311_");
    expect(baseline).toContain("migration 0311");
  });
});
