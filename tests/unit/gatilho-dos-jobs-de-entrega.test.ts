...
const GATILHO_ESPERADO: Record<string, { condicao: string | null; efeito: string }> = {
  ...
  "sincronizar-upstream.yml::verificar-e-trazer": {
    condicao: null,
    efeito:
      "Cron diário (17 6 * * *) que confere o upstream (melgarafael/DeskcommCRM), traz commits " +
      "novos para a branch `sync-upstream` e abre PR se o merge for limpo — ou Issue se colidir. " +
      "Sem este job rodando, o fork acumula divergência do upstream em silêncio: ninguém percebe " +
      "que há semanas/meses de correções não incorporadas até um merge manual muito maior e mais " +
      "arriscado precisar acontecer.",
  },
};
