...
describe("a marca padrão do produto", () => {
  ...
  it("é o que aparece quando o operador não configurou marca nenhuma", () => {
    ...
    expect(resolveBranding(undefined, undefined), COMO_PERSONALIZAR).toEqual({
      ...
      logoUrl: null,
      initial: "D",
      // Este fork estende `resolveBranding` com cor de destaque (accentColor +
      // o texto legível em cima dela) — upstream ainda não tem o campo. Sem
      // accentColor configurado (o caso deste teste), os dois saem `null`.
      accentColor: null,
