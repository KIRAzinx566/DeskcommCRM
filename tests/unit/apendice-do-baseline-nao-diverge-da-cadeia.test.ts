...
function semComentarios(sql: string): string {
  return (
    sql
      // `\r\n` e não só `\n`: um checkout Windows com `core.autocrlf=true`
      // entrega as migrations em CRLF enquanto o baseline.sql fica em LF. Sem
      // isto, cada linha carrega um `\r` residual depois de `.*` (que não
      // cruza terminador de linha), o `$` sem `/m` nunca alcança o fim real da
      // string, e `--.*$` deixa de casar — o comentário sobrevive à
      // "remoção" e 21 funções idênticas acusam divergência que não existe.
      .split(/\r?\n/)
      .map((l) => l.replace(/--.*$/, ""))
      .join(" ")
