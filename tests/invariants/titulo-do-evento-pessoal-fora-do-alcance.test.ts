...
/**
 * O TÍTULO DO EVENTO PESSOAL DO GOOGLE NÃO ALCANÇA OUTRO MEMBRO — migration 0268.
 *
 * ─── O defeito ──────────────────────────────────────────────────────────────
 *
 ...
 * 2. O CAMINHO DE ATUALIZAÇÃO: o bloco da 0268, LIDO do `supabase/baseline.sql`
 ...
 *    a `authenticated`: os casos do item 2 reaplicam a 0268 e a reaplicação
 ...
 * 0268); se vier, só o caso "o que a 0268 NÃO fecha" fica vermelho, de propósito.
 */

const BASELINE = readFileSync(join(process.cwd(), "supabase", "baseline.sql"), "utf8");

const ROTULO_0268 =
  "-- ---- PRIVACIDADE: o título do evento pessoal do Google sai do alcance do membro (migration 0268) ----";

/** O bloco rotulado da 0268, do rótulo até o próximo rótulo de apêndice. */
function blocoDa0268(): string {
  const inicio = BASELINE.indexOf(ROTULO_0268);
  if (inicio === -1) throw new Error("rótulo da 0268 não encontrado no baseline");
  if (BASELINE.indexOf(ROTULO_0268, inicio + 1) !== -1) throw new Error("rótulo da 0268 repetido no baseline");
  const fim = BASELINE.indexOf("\n-- ---- ", inicio + ROTULO_0268.length);
  if (fim === -1) throw new Error("fim do bloco da 0268 não encontrado");
 ...
 *   da 0268 CRIA a view (`drop` + `create`). É ele que dá tudo a `anon` na view
...
const ORG = "02680000-0000-4000-8000-0000000000a1";
const DONO = "02680000-0000-4000-8000-000000000001";
const COLEGA = "02680000-0000-4000-8000-000000000002";
const CONEXAO = "02680000-0000-4000-8000-0000000000c1";
const EVENTO = "02680000-0000-4000-8000-0000000000e1";
/** A conta Google do `DONO` — e o id da agenda principal dele no Google. */
const EMAIL_DO_DONO = "dono-0268@invariant.test";
 ...
 */
const ICAL_UID_RESIDUAL = "convite-0268@sistema-de-quem-convidou.test";
...
const FIXTURE = `
  insert into auth.users (id, email) values
    ('${DONO}', '${EMAIL_DO_DONO}'),
    ('${COLEGA}', 'colega-0268@invariant.test');
  insert into public.organizations (id, slug, legal_name, display_name)
    values ('${ORG}', 'inv-0268', 'Inv 0268', 'Inv 0268');
  ...
  insert into public.calendar_external_events
    ...
    values ('${EVENTO}', '${ORG}', '${CONEXAO}', 'pessoal', 'ev-0268', '${TITULO}', '${ICAL_UID_RESIDUAL}',
...
const GESTOR = "02680000-0000-4000-8000-000000000003";
const GESTOR_NA_ORG = `
  insert into auth.users (id, email) values ('${GESTOR}', 'gestor-0268@invariant.test');
...
const CALENDARIO = "02680000-0000-4000-8000-0000000000d1";
...
const SINCRONIZA_COMO_SERVICE_ROLE = `
  ...
  select public.fn_google_calendar('${ORG}', '${CALENDARIO}', 'item', jsonb_build_object(
    'claim', :'reserva'::jsonb,
    'item', jsonb_build_object('external_event_id', 'ev-0268', 'title', '${TITULO} (do Google)',
  ...
  select public.fn_google_calendar('${ORG}', '${CALENDARIO}', 'item', jsonb_build_object(
    'claim', :'reserva'::jsonb,
    'item', jsonb_build_object('external_event_id', 'ev-0268-novo', 'title', 'Entrevista de emprego',
...
describe("migration 0268 — o título do evento pessoal fora do alcance do membro", () => {
  ...
  it("sobre a v1.26.0, o bloco da 0268 dá permission denied no título ao colega — o erro, não zero linhas", () => {
  ...
  it("sobre a v1.26.0, o bloco da 0268 tira o SELECT de tabela e mantém a ocupação concedida", () => {
  ...
  it("sobre a v1.26.0, o bloco da 0268 deixa de pé quem mostra a ocupação: a leitura da tela para o dono e o gestor, e a função da 0260 para o colega — cuja tela já não trazia a ocupação do dono", () => {
    ...
    // gestor, a tela JÁ não mostrava a ocupação do Google do colega antes da 0268
    ...
    // 0268). O que precisa continuar de pé é o que este caso mede.
    ...
    expect(
      colegaAntes,
      "na v1.26.0 a leitura da tela entregou ao colega que não é gestor a ocupação do dono — a premissa com que a 0268 descreve a grade mudou",
    ).toBe("tela=0");
    expect(
      colegaDepois,
      "depois do bloco a leitura da tela entregou ao colega que não é gestor a ocupação do dono — a premissa com que a 0268 descreve a grade mudou",
    ).toBe("tela=0");
  ...
  it("o bloco não fecha o espelho para quem o mantém: o sincronizador, que o service_role chama, grava a ocupação com o título NULO — zerando o que encontra — e a desconexão, como service_role, apaga", () => {
    // O que a 0268 fecha é a LEITURA por login de usuário. Os dois caminhos que
    ...
    // ser mantido.
    //
    // E o caso prende a premissa que a prosa da 0268 usa para dizer o alcance: o
    ...
    //
    // Prende também o que a 0268 diz do `ical_uid`, do mesmo período: o
    ...
    expect(
      linhas,
      "o service_role perdeu o que os caminhos do produto usam (EXECUTE do sincronizador, DELETE/SELECT da desconexão), ou o sincronizador mudou o que grava — voltou a gravar o nome, ou passou a gravar/limpar o ical_uid (a prosa da 0268 descreve o resíduo)",
    ).toEqual([
      "papel=service_role",
      "executa=true",
      `espelho=ev-0268:(nulo):${ICAL_UID_RESIDUAL},ev-0268-novo:(nulo):(nulo)`,
  ...
  it("o que a 0268 NÃO fecha, e declara: o colega segue lendo o id do calendário — na agenda principal, o e-mail da conta que a RLS da conexão esconde dele", () => {
    ...
    //
    // A 0268 deixa isso aberto por decisão escrita no cabeçalho dela. Revogar a
    ...
    // dia em que alguém fechar, ele fica vermelho — e a 0268, o fragmento e o
    ...
    const [conexao, tabela, view] = sondasDesfeitas(`
      ...
      insert into public.calendar_external_events
        ...
        values ('${ORG}', '${CONEXAO}', '${EMAIL_DO_DONO}', 'ev-0268-principal',
      ...
      select '${MARCA}' || 'tabela=' || external_calendar_id
        from public.calendar_external_events where external_event_id = 'ev-0268-principal';
      select '${MARCA}' || 'view=' || external_calendar_id
        from public.calendar_selected_external_events where external_event_id = 'ev-0268-principal';
    `);
    ...
    expect(
      tabela,
      "o colega deixou de ler o id do calendário na tabela — a exposição declarada na 0268 foi fechada: atualize a prosa",
    ).toBe(`tabela=${EMAIL_DO_DONO}`);
    expect(
      view,
      "o colega deixou de ler o id do calendário pela view — a exposição declarada na 0268 foi fechada: atualize a prosa",
    ).toBe(`view=${EMAIL_DO_DONO}`);
  });
});

/**
 * O RESÍDUO que a ressincronização deixa — o que a 0268 escreve sobre "o alcance
 ...
 */
describe("o alcance do resíduo que a 0268 descreve — o que a ressincronização não regrava", () => {
  it("o rebuild completo de 24h não regrava nem apaga o evento CANCELADO: futuro e dentro da janela, ele mantém o nome — e o evento confirmado que sumiu do Google é apagado", () => {
    ...
    const [modo, espelho] = sondasDesfeitas(`
      ...
      insert into public.calendar_external_events
        ...
        values
        ('${ORG}', '${CONEXAO}', 'pessoal', 'ev-0268-cancelado', 'Consulta cancelada', 'cancelled',
         now() + interval '3 days', now() + interval '3 days 1 hour'),
        ('${ORG}', '${CONEXAO}', 'pessoal', 'ev-0268-sumiu', 'Entrevista que sumiu', 'confirmed',
      ...
      select public.fn_google_calendar('${ORG}', '${CALENDARIO}', 'page', jsonb_build_object(
        'claim', :'reserva'::jsonb, 'next_page_token', null, 'next_sync_token', 'sync-0268'));
      reset role;
      select '${MARCA}' || 'espelho=' ||
    ...
    expect(
      espelho,
      "o rebuild passou a regravar/apagar o evento cancelado, ou deixou de apagar o confirmado que sumiu — a prosa do resíduo na 0268 muda junto",
    ).toBe("espelho=ev-0268:confirmed:(nulo),ev-0268-cancelado:cancelled:Consulta cancelada");
  });
  ...
  it("a agenda que o sincronizador não lê não é regravada, futuro inclusive: fora do catálogo a reserva não sai, e de membro revogado ou de conexão com o token vencido ela é recusada", () => {
    ...
    expect(foraDoCatalogo, "o sincronizador passou a reservar agenda fora do catálogo — a prosa do resíduo na 0268 muda junto").toBe(
      "reserva=(nenhuma)",
    );
    ...
    const membroRevogado = erroDo(`
      ...
      rollback;
    `);
    expect(membroRevogado, "o sincronizador passou a reservar a agenda de membro revogado — a prosa do resíduo na 0268 muda junto").not.toBeNull();
    ...
    const tokenVencido = erroDo(`
      ...
      rollback;
    `);
    expect(tokenVencido, "o sincronizador passou a reservar agenda de conexão fora do ar — a prosa do resíduo na 0268 muda junto").not.toBeNull();
    expect(tokenVencido).toContain("google_connection_unavailable");
  });
});

describe("o banco instalado pelo baseline inteiro — sem reaplicar o bloco da 0268", () => {
  it("não entrega o título ao colega: sem SELECT de tabela, sem SELECT no `title`, e a view sem a coluna", () => {
    // Todo caso acima reaplica a 0268, e a reaplicação revoga de novo. Um bloco
  ...
  it("nenhuma função nem view que `authenticated` ou `anon` alcance lê o título do espelho — quem lê com o privilégio do dono passa por cima do grant de coluna", () => {
    ...
    const [tituloPeloColega, citamComSondas, alcancaveisComSondas] = sondasDesfeitas(`
      ${FIXTURE}
      create function public.fn_sonda_0268_titulo_do_titular(p_conexao uuid) returns setof text
        language sql stable security definer set search_path = public
        as $f$ select e.title from public.calendar_external_events e where e.connection_id = p_conexao $f$;
      create function public.fn_sonda_0268_linha_inteira(p_conexao uuid) returns setof jsonb
        language sql stable security definer set search_path = public
        as $f$ select to_jsonb(e) from public.calendar_external_events e where e.connection_id = p_conexao $f$;
      create view public.v_sonda_0268_espelho as select e.* from public.calendar_external_events e;
      create function public.fn_sonda_0268_so_ocupacao(p_conexao uuid) returns setof timestamptz
        language sql stable security definer set search_path = public
        as $f$ select e.starts_at from public.calendar_selected_external_events e where e.connection_id = p_conexao $f$;
      create function public.fn_sonda_0268_titulo_do_servico(p_conexao uuid) returns setof text
        language sql stable security definer set search_path = public
        as $f$ select e.title from public.calendar_external_events e where e.connection_id = p_conexao $f$;
      revoke execute on function public.fn_sonda_0268_titulo_do_titular(uuid) from public, anon;
      grant  execute on function public.fn_sonda_0268_titulo_do_titular(uuid) to authenticated;
      revoke execute on function public.fn_sonda_0268_linha_inteira(uuid) from public, anon;
      grant  execute on function public.fn_sonda_0268_linha_inteira(uuid) to authenticated;
      revoke all on public.v_sonda_0268_espelho from public, anon;
      grant  select on public.v_sonda_0268_espelho to authenticated;
      revoke execute on function public.fn_sonda_0268_so_ocupacao(uuid) from public, anon;
      grant  execute on function public.fn_sonda_0268_so_ocupacao(uuid) to authenticated;
      revoke execute on function public.fn_sonda_0268_titulo_do_servico(uuid) from public, anon, authenticated;
      grant  execute on function public.fn_sonda_0268_titulo_do_servico(uuid) to service_role;
      ${COMO_COLEGA}
      select '${MARCA}' || 'colega=' || string_agg(t, ',') from public.fn_sonda_0268_titulo_do_titular('${CONEXAO}') t;
      reset role;
    ...
    expect(citamComSondas).toBe(
      "citam=função fn_google_calendar(uuid,uuid,text,jsonb),função fn_sonda_0268_linha_inteira(uuid)," +
        "função fn_sonda_0268_titulo_do_servico(uuid),função fn_sonda_0268_titulo_do_titular(uuid),view v_sonda_0268_espelho",
    );
    expect(alcancaveisComSondas, "a varredura não pega um leitor alcançável do título, ou pega quem só lê ocupação").toBe(
      "alcancaveis=função fn_sonda_0268_linha_inteira(uuid),função fn_sonda_0268_titulo_do_titular(uuid),view v_sonda_0268_espelho",
    );
  });
  ...
  it("coluna do espelho legível por membro é decisão explícita: só o `title` fica de fora, e a view cabe no que foi concedido", () => {
    ...
    // Isso está declarado na 0268 e medido pelo último caso do describe da
...
describe("sob o default ACL do Supabase, simulado na transação antes do bloco da 0268", () => {
  ...
  it("a view recriada pelo bloco não fica legível por `anon`, embora o default ACL do Supabase dê tudo a `anon` em relação nova", () => {
    ...
    const [view] = sondasDesfeitas(`
      ...
      select '${MARCA}' || has_table_privilege('anon', 'public.calendar_selected_external_events', 'SELECT')::text;
    `);
    expect(view, "com o default ACL do Supabase, `anon` lê a view da ocupação recriada pela 0268").toBe("false");
  });
  ...
  it("o dono lê a ocupação que a tela dele lê — e também não lê o título, porque nenhuma tela o mostra", () => {
    ...
    const titulo = erroDo(`
      ...
      rollback;
    `);
    expect(titulo, "o dono lê o título pela sessão — o grant não é o da 0268").not.toBeNull();
  ...
  it("fn_agenda_ocupacao_google_do_dono (migration 0260) segue devolvendo a ocupação por cima da view recriada", () => {
    ...
    // uma `security definer`. O `drop view` da 0268 não pode deixá-la sem chão.
