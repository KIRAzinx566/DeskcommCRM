...
--
--   · 0382 (renumerada de 0312) refez `agent_inbox_items_kind_check` SEM `aviso_de_caso_nao_entregue`
...
-- via. O defeito é do ENCONTRO, e por isso o conserto é forward-fix — editar 0382 (renumerada de 0312)
...
--   CHECK ............. os 28 valores de 0382 (renumerada de 0312) + `aviso_de_caso_nao_entregue` = 29
--   fn_atrito_metrics . o corpo de 0322 (com `envios_por_automacao`,
--                       `envios_por_integracao` e a regra de
--                       `moved_to_another_pipeline` da 0311, renumerada de 0266) MAIS o bloco do laço
--                       de retorno da 0294.
--
...
alter table public.agent_inbox_items
  add constraint agent_inbox_items_kind_check check (kind in (
    ...
    -- (migration 0382, renumerada de 0312) O fluxo de follow-up publicado que NUNCA vai disparar:
...
as $$
  ...
  eficiencia as (
    select count(*) filter (where status = 'won')  as ganhos,
           count(*) filter (
             where status = 'lost'
               -- A transferência entre funis não é perda comercial (migration 0311, renumerada de 0266).
