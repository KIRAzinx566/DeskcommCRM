...
- **Depois do agente nascer, um segundo pacote de capacidades volta a caber** O catálogo de capacidades cresceu na sincronização com o upstream, e o teto de 25 capacidades por agente voltou a ficar apertado demais: a partir do agente que nasce no onboarding, nenhum pacote de jornada (Atender, Escalar, Organizar, Evoluir ou Reter) cabia mais no teto — o dono ligava o primeiro pacote e o produto passava a recusar todos os outros, com o checkbox simplesmente desabilitado, sem explicação.

  O teto sobe para 26 — o menor passo que volta a abrir pelo menos um pacote (Evoluir). Você não precisa fazer nada.
...
<!--
  ...
  não se somam aos daqui embaixo (1.13.0 e anteriores, deste fork). Trazido por
  este merge para preservar o histórico; a próxima release deste fork continua
  contando a partir da SUA própria sequência (.changes/ + scripts/cortar-release.ts),
  nunca a partir destes números.

  O "-upstream" no 1.13.0 é deliberado, mesma lógica do "-kirazinx" mais abaixo
  no arquivo (ver o comentário perto de 1.11.1-kirazinx): o upstream cortou a
  própria 1.13.0 no mesmo período em que este fork cortou a SUA — duas releases
  de repositórios diferentes, mesmo número, conteúdo totalmente diferente. Sem o
  sufixo, a seção `## [1.13.0]` deste arquivo colidiria com a que já existe
  abaixo (a real, marcada por `v1.13.0` no GitHub deste fork) e um leitor não
  saberia qual delas foi de fato publicada aqui.
-->

<!--
  Bloco abaixo (1.23.0 a 1.13.0-upstream): releases do UPSTREAM (melgarafael/DeskcommCRM)
  ainda não incorporadas a este fork. Numeração DELE, não deste fork — os números
  não se somam aos daqui embaixo (1.13.0 e anteriores, deste fork). Trazido por
  este merge para preservar o histórico; a próxima release deste fork continua
  contando a partir da SUA própria sequência (.changes/ + scripts/cortar-release.ts),
...
- **A chave da OpenRouter passa a ser conferida de verdade antes de a tela dizer que está validada** Ao cadastrar uma chave da OpenRouter em **Agente de IA › Credenciais**, o sistema conferia
  ...
  **1.13.0-upstream**. O que chega atrasado é esta nota — a mudança foi publicada sem ela,
  ...
  OpenRouter é anterior à 1.13.0-upstream e o atendimento falha sem motivo aparente, abra
...
- **Agente em provider "API customizada" não conseguia ter um modelo escolhido** O seletor de modelo do editor de agente (`ModelPicker`) só sabia ser uma
...
<!--
  As duas seções abaixo (1.11.1-kirazinx / 1.11.0-kirazinx) são releases DESTE
  FORK, cortadas em paralelo ao upstream — por isso o sufixo: o número puro
  colidiria com o 1.11.1/1.11.0 do upstream logo abaixo, que é conteúdo
  DIFERENTE. As tags reais no GitHub (v1.11.0, v1.11.1) já existem sem o
  sufixo; ele é só a chave que desambigua as duas seções aqui dentro.
-->

## [1.11.1-kirazinx] — 2026-09-01
...
- **Agentes com NVIDIA aguentam o limite de requisições sem desistir** O tier gratuito da NVIDIA aceita só 40 requisições por minuto por modelo, e uma
...
[Não lançado]: https://github.com/KIRAzinx566/DeskcommCRM/compare/v1.18.0...HEAD
[1.18.0]: https://github.com/KIRAzinx566/DeskcommCRM/compare/v1.17.0...v1.18.0
[1.17.0]: https://github.com/KIRAzinx566/DeskcommCRM/compare/v1.16.0...v1.17.0
...
[1.16.0]: https://github.com/KIRAzinx566/DeskcommCRM/compare/v1.15.0...v1.16.0
...
[1.14.0]: https://github.com/melgarafael/DeskcommCRM/compare/v1.13.0-upstream...v1.14.0
[1.13.0-upstream]: https://github.com/melgarafael/DeskcommCRM/compare/v1.12.0...v1.13.0-upstream
...
[1.11.0-kirazinx]: https://github.com/KIRAzinx566/DeskcommCRM/compare/v1.10.4...v1.11.0
