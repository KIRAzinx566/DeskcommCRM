...
function* arquivosDeTeste(raiz: string, dir: string): Generator<string> {
  for (const entrada of readdirSync(dir, { withFileTypes: true })) {
    ...
    // `relative()` usa separador do SO — no Windows isso quebra todo `.startsWith`/
    // `.includes` contra os caminhos com `/` que o resto do repo assume (aqui e em
    // `tests/unit/cercas-sao-selecionadas-pelo-import.test.ts`).
    else if (/\.test\.tsx?$/.test(entrada.name)) yield relative(raiz, caminho).split(sep).join("/");
  }
}
