...
Migration 0234 dá a `organizations` uma coluna `currency`; o resto do frente
...
novo). Conferido: a migration 0234 não usa nada exclusivo de pg17 (só `ADD
...
Configurações › Tipos de agendamento (migration 0269). A porta secundária é o
...
| Caso | Prioridade | Resultado |
|---|---|---|
| L11.1 O banco recém-instalado pelo baseline já traz as três colunas da 0269, os quatro gatilhos, as três funções, a view **sem** `title` e `has_column_privilege(authenticated, …, title, SELECT) = false` | `[P0]` | **PASS** — tabela no README |
